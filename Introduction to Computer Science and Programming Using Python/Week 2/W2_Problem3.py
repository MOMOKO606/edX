# -*- coding: utf-8 -*-
"""
Created on Sat Jul  7 15:56:56 2018
...
slowly, especially in cases with very large balances and interest rates. (Note: 
    
when your code is running on our servers, there are limits on the amount of computing 

time each submission is allowed, so your observations from running this experiment 
...
def RemaBalance2(balance, annualInterestRate, monthlyPayment, mon):
    """
    Input parameters:
    balance - the outstanding balance on the credit card
    annualInterestRate - annual interest rate as a decimal
    monthlyPayment - a constant integer
    ...
    rb = [balance]  
    for i in range(mon):
        # Monthly unpaid balance = (Previous balance) - (Monthly payment)
        mub = rb[i] - monthlyPayment
...
while low < high:
    mid = ( low + high ) / 2
    rebalance = RemaBalance2(balance, annualInterestRate, mid, 12)
    if rebalance == 0:
        break
    if rebalance > 0:
        low = mid + 0.01
